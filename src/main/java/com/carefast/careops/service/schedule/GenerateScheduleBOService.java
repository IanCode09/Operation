...
@Service
public class GenerateScheduleBOService {
    public int dayOff = 4;
    ...
    public boolean generateCsoSchedule(String projectCode, int month, int year) {
            ...
            if (yearMonth.lengthOfMonth() == 28) {
                if (i == 1 || x == 1) {
                    ...
                    if (i <= morningShiftManPower) {
                        scheduleRepository.save(saveScheduleModelFirstRowTypeOne(projectCode, month, year, morningShiftId));
                    } else if (i > morningShiftManPower) {
                        scheduleRepository.save(saveScheduleModelFirstRowTypeOne(projectCode, month, year, afternoonShiftId));
                    }
                } else {
                    ScheduleModel scheduleExist = scheduleRepository.getLastSchedule();

                    if (i <= morningShiftManPower) {
                        scheduleRepository.save(saveScheduleModelTypeOne(scheduleExist, projectCode, month, year, morningShiftId));
                    } else if (i <= (afternoonShiftManPower + morningShiftManPower)) {
                        scheduleRepository.save(saveScheduleModelTypeOne(scheduleExist, projectCode, month, year, afternoonShiftId));
                    } else if (i <= (nightShiftManPower + afternoonShiftManPower + morningShiftManPower)) {
                        scheduleRepository.save(saveScheduleModelTypeOne(scheduleExist, projectCode, month, year, nightShiftId));
                    }
                }
                ...
                if (i == 1 || x == 1) {
                    ...
                    // FIRST ROW
                    if (i <= morningShiftManPower) {
                        scheduleRepository.save(saveScheduleModelFirstRowTypeTwo(projectCode, month, year, morningShiftId));
                    } else if (i > morningShiftManPower) {
                        scheduleRepository.save(saveScheduleModelFirstRowTypeTwo(projectCode, month, year, afternoonShiftId));
                    }
                } else {
                    ScheduleModel scheduleExist = scheduleRepository.getLastSchedule();

                    if (i <= morningShiftManPower) {
                        scheduleRepository.save(saveScheduleModelTypeTwo(scheduleExist, projectCode, month, year, morningShiftId));
                    } else if (i > morningShiftManPower) {
                        scheduleRepository.save(saveScheduleModelTypeTwo(scheduleExist, projectCode, month, year, afternoonShiftId));
                    }
                }
                ...
                if (i == 1 || x == 1) {
                    ...
                    // FIRST ROW
                    if (i <= morningShiftManPower) {
                        scheduleRepository.save(saveScheduleModelFirstRowTypeThree(projectCode, month, year, morningShiftId));
                    } else if (i > morningShiftManPower) {
                        scheduleRepository.save(saveScheduleModelFirstRowTypeThree(projectCode, month, year, afternoonShiftId));
                    }
                } else {
                    ScheduleModel scheduleExist = scheduleRepository.getLastSchedule();

                    if (i <= morningShiftManPower) {
                        scheduleRepository.save(saveScheduleModelTypeThree(scheduleExist, projectCode, month, year, morningShiftId));
                    } else if (i > morningShiftManPower) {
                        scheduleRepository.save(saveScheduleModelTypeThree(scheduleExist, projectCode, month, year, afternoonShiftId));
                    }
                }
            } else {
                if (i == 1 || x == 1) {
                    ...
                    // FIRST ROW
                    if (i <= morningShiftManPower) {
                        scheduleRepository.save(saveScheduleModelFirstRowTypeFour(projectCode, month, year, morningShiftId));
                    } else if (i > morningShiftManPower) {
                        scheduleRepository.save(saveScheduleModelFirstRowTypeFour(projectCode, month, year, afternoonShiftId));
                    }
                } else {
                    ScheduleModel scheduleExist = scheduleRepository.getLastSchedule();

                    if (i <= morningShiftManPower) {
                        scheduleRepository.save(saveScheduleModelTypeFour(scheduleExist, projectCode, month, year, morningShiftId));
                    } else if (i > morningShiftManPower) {
                        scheduleRepository.save(saveScheduleModelTypeFour(scheduleExist, projectCode, month, year, afternoonShiftId));
                    }
                }
            }
        }
